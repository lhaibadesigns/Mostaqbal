...
{
  const process = document.querySelector('.serviceSectionContainer');
  if ((typeof(process) != 'undefined' && process != null)) {
    let sections = gsap.utils.toArray('.serviceSectionContainer section');

    gsap.to(sections, {
      xPercent: -125 * (sections.length - 1),
      ease: "none",
      scrollTrigger: {
        trigger: process,
        markers: false,
        ...
        end: () => "+=" + document.querySelector(".serviceSectionContainer").offsetWidth
      },
    });
  }
}
    // End Services Section Scroll